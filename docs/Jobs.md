...
- **域级入口**（项目根）：按业务域拆分脚本，内建有序 **`JOBS_REGISTRY`**（`job_name → run`），**`main()`** 按注册顺序依次执行；**cron** 宜分别指向各域入口，例如 **`update_trade_calendar.py`**（交易日历）、**`update_security_info.py`**（证券信息如 ETF）、**`update_daily_trade_data.py`**（日线交易数据）、**`update_minutely_trade_data_morning.py`** / **`update_minutely_trade_data_afternoon.py`**（分钟交易数据），或使用 `python -m` 等价入口。
- 运行前需保证 **`PYTHONPATH` 含项目根**（或先调用 **`utils.add_root_path()`**，与现有脚本一致）。

## 执行结果与通知（JobInfo）

- 各 **`run()`** 宜返回 **`JobInfo`**（见 **`models/job_info/job_info.py`**）：含 **`job_name`**、**`finished_at`**、**`success`**、**`error`**、**`write_times`**、**`write_failed_times`**、**`total_fetch_times`**、**`fallback_records`**、**`additional_info`** 等，供域级入口拼 **汇总邮件** 或落日志。
- **字段分工（约定）**：
...
- 纯文本邮件正文可由 **`models/job_info/template.py` → `jobinfo_to_email_body()`** 将单条 **`JobInfo`** 格式化为一段；多 job 时在入口中拼接。发信 **`send_email()`** 仍可能抛出 **`EmailError`**，入口宜 **`try/except EmailError`** **仅记日志、不二次发信**（见 **`docs/Error.md`**）。

## 当前示例

| 模块 | 说明 |
| ---- | ---- |
| `models/job_info/job_info.py` | **`JobInfo`**（`TypedDict`）定义 |
