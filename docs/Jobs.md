...
| `update_daily_data.py` | 日线数据域：顺序执行 **`update_etf_daily_trade_data`**、**`update_stock_daily_trade_data`**（TuShare 股票日线）、**`update_etf_adjustment_factor`**；汇总 **`JobInfo`** 后发邮件 |
...
| `jobs/adjustment_factor/update_etf_adjustment_factor.py` | 更新 **`adjustment_factor.db`** → **`etf_adjustment_factor`**；**`run()`** 返回 **`JobInfo`** |
