...
logger = get_logger('update_stock_info_job')
...
def run()->JobInfo:
    ...
    # 条件判断
    days_interval = (dt.date.today() - stock_info.get_latest_update_date()).days
    ...
    if condition:
        try:
            df, fallback_records, fetch_times = stock_info_by_mairui.provide()
        ...
        try:
            flushed = stock_info_buffer.append(df)
            if flushed:
                write_times += 1
        ...
        # 最后flush缓存
        try:
            flushed = stock_info_buffer.flush()
            if flushed:
                write_times += 1
        except BufferWriteError as e:
            write_times += 1
